...
> 状态：设计定稿，未实现
> 日期：2026-08-19
> 目标运行时：DeepSeek Harness（dsh）0.1.x
...
角色定义可改绑定、可增删自定义角色。三个内置角色提供语义区分度：三个工具的 `description` 各不相同，模型能够判断该用哪个；纯 alias 路由表下所有工具描述高度相似，模型选择会退化。
...
- **skill 正文尚未撰写**。四份方法论文本是本产品的核心资产，篇幅与质量决定产品价值，需在实施阶段逐份撰写并验证。
