...
/**
 * 候选主体里额外排除的三个 Unicode 区块（区块端点已用 `codePointAt` 逐字符核对
 * 过，避免宽度相近符号被误粘贴成别的字符）：
 * - U+2018–U+201F（弯引号，含 “”‘’）
 * - U+3000–U+303F（CJK 符号与标点，含 。、「」『』【】〔〕《》）
 * - U+FF00–U+FFEF（半角及全角字符，含 ：，；（）等全角标点）
 *
 * 三个区块只覆盖标点/符号，不含 CJK 统一表意文字（U+4E00–U+9FFF），普通中文
 * 文件名/目录名字符不受影响。
 */
const FULLWIDTH_PUNCTUATION = '‘-‟　-〿＀-￯'

/**
 * 匹配候选路径片段：非空白、非中文全角标点，且含有路径分隔符或 `.md` 后缀的
 * 连续串。前后可被反引号、单双引号、圆括号包裹，捕获时不含包裹符。
 *
 * 候选主体排除 {@link FULLWIDTH_PUNCTUATION} 列出的字符：协调者模型用自然中文
 * 转述路径时，全角标点常常紧贴路径书写（例如"计划文件：xxx.md"、"xxx.md
 * （说明）"）；不排除的话会被贪婪并入候选串，`resolve()` 出的路径带有多余前后缀
 * 而无法通过围栏检查，导致一个真实存在、合法落在 plansDir 内的路径被误判为
 * 不存在/不在围栏内而拒绝（2026-08-19 真实客户端联调复现）。
 */
const CANDIDATE = new RegExp(
  '[`\'"(]?([^\\s`\'"()' + FULLWIDTH_PUNCTUATION + ']*[/\\\\][^\\s`\'"()' + FULLWIDTH_PUNCTUATION + ']*'
  + '|[^\\s`\'"()' + FULLWIDTH_PUNCTUATION + ']+\\.md)[`\'")]?',
  'g',
)
