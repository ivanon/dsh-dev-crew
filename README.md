...
**三、等待期内提问** —— 派发任一角色工具后，在做出实质工作之前，`ask_user_question`
一律拒绝。`list_agents` 与提问本身都不解除等待期（它们正是编排者用来假装等待的两个
工具），任何其他工具调用（读文件、改文件、跑命令）解除。

这一条针对最难拦的一种假等待：问题带着两个像样的选项（「继续等待」/「中止流水线」），
既不是空列表，也不与派发相邻。真实会话里编排者在同一个 turn 里问了四次「子代理长时间
未返回，你希望继续等待还是中止？」——把等待责任推给用户，而流水线承诺规格落盘后零打
扰。需求讨论环节的连续提问不受影响，那时还没派出任何子代理。

**四、重复派发同一 reviewer** —— 同一个 `subagent_reviewer*` 工具连续派发第二次即
